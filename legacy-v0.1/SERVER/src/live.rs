...
pub enum Execution {
    Paper(PaperBroker),
    Alpaca { broker: AlpacaBroker, tracked_units: f64, entry_equity: f64, peak_equity: f64 },
}
...
pub async fn run_session(session: Arc<Session>, settings: Settings, strategy: Strategy) {
    ...
    session.emit("session_start", json!({
    ...
    while !session.stop.load(Ordering::Relaxed) {
        match stream_ticks(&session, &settings, &strategy, &mut bars, &mut cur).await {
            ...
            Err(e) => {
                if session.stop.load(Ordering::Relaxed) { break; }
...
async fn stream_ticks(
    ...
    loop {
        if session.stop.load(Ordering::Relaxed) { return Ok(()); }
        let msg = tokio::time::timeout(std::time::Duration::from_secs(60), ws.next())
            .await.map_err(|_| "ws receive timeout".to_string())?;
        let Some(msg) = msg else { return Err("ws closed".into()) };
        let msg = msg.map_err(|e| e.to_string())?;
        let Message::Text(text) = msg else { continue };
        let v: serde_json::Value = match serde_json::from_str(&text) { Ok(v) => v, Err(_) => continue };
        if v.get("type").and_then(|t| t.as_str()) != Some("ticker") { continue; }
        let Some(price) = v.get("price").and_then(|p| p.as_str()).and_then(|p| p.parse::<f64>().ok())
        else { continue };
        ...
        let closed_bar = match cur {
            ...
            _ => {
                let prev = cur.take();
                *cur = Some(Candle { time: now, open: price, high: price, low: price,
        ...
        if let Some(bar) = closed_bar {
            bars.push_back(bar);
            while bars.len() > session.cfg.window_bars { bars.pop_front(); }
...
async fn on_bar_close(
    ...
    let (dd, kill_limit) = match &mut st.execution {
        ...
        Execution::Alpaca { tracked_units, entry_equity, peak_equity, .. } => {
            let eq = *entry_equity + *tracked_units * price;
            if eq > *peak_equity { *peak_equity = eq; }
            (if *peak_equity > 0.0 { eq / *peak_equity - 1.0 } else { 0.0 }, session_kill_limit())
        }
    };
    if dd <= -kill_limit {
        st.halted = true;
    ...
    if bars.len() < session.cfg.window_bars / 2 { return Ok(()); }
    ...
    match &mut st.execution {
        ...
        Execution::Alpaca { broker, tracked_units, entry_equity, .. } => {
            ...
            if notional.abs() >= 1.0 {
                ...
                match broker.order_notional(&session.cfg.symbol, notional, &journal).await {
                    Ok(()) => {
...
fn session_kill_limit() -> f64 {
    std::env::var("PRISMATIK_MAX_DD_KILL").ok()
