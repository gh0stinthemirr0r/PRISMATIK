    ...
    pub fn from_spec(spec: &StrategySpec) -> Result<Self, String> {
        ...
        let s = match spec.kind.as_str() {
            ...
            "donchian" => {
                ...
                if lookback < 2 {
                    return Err("lookback must be >= 2".into());
                }
            ...
            "rsi" => {
                ...
                if period < 2 {
                    return Err("period must be >= 2".into());
                }
...
pub fn apply_vol_target(
    ...
    for i in 0..weights.len() {
        ...
        let scale = if ann > 0.0 {
            (target_annual_vol / ann).min(max_scale)
        } else {
            0.0
        };
...
mod tests {
    ...
    #[test]
    fn spec_validation() {
        let bad = StrategySpec {
            kind: "ma".into(),
            params: serde_json::json!({"fast": 96, "slow": 24})
                .as_object()
                .unwrap()
                .clone(),
            vol_target: None,
        ...
        assert!(Strategy::from_spec(&StrategySpec {
            kind: "nope".into(),
